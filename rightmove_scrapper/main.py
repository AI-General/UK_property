...
def get_detailed_info(i, property_list):
    ...
    append_dict_to_csv(property_dict, f"rightmove_data.csv")
...
async def run():
    ...
    print("number of properties in first half: ",len(first_half))
    for index, property_id in enumerate(first_half):
        try:
            property_data = await scrape_properties([f"https://www.rightmove.co.uk/properties/{property_id}#/"])
            if property_data == []:
                print(f"Property data for  {property_id} is empty!")
                not_found_list.append(property_id)
                continue

            get_detailed_info(index, property_data)
        except Exception as e:
            print(f"Error occurred while scraping property {property_id}: {e}")
            not_found_list.append(property_id)
        
    with open("not_found_list.txt", "w") as f:
        f.write(str(not_found_list))
